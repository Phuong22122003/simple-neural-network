...
class Model:
    ...
    def fit(self, inputs, outputs, epochs = 1, batch_size=32, learning_rate=1e-4, to_one_hot = False):
        ...
        for epoch in range(epochs):
            ...
            pbar = tqdm(range(0, inputs.shape[0], batch_size), desc=f"Epoch {epoch+1}", unit="batch")
            learning_rate*=0.9
            for i in pbar:
                inputs_batch = inputs[i:i+batch_size, :] 
                if(to_one_hot):
                    outputs_batch = self.to_one_hot(outputs[i:i+batch_size, :]   ,self.layers[-1].output_dim)
                else:
